...
class ExtractDataDomainTor(DiscoverableTransform):
    """
    Transform to extract emails and cryptocurrency wallets from a given Tor domain.
    """

    @classmethod
    def create_entities(cls, request: MaltegoMsg, response: MaltegoTransform):
        domain = request.Value.strip()
        url = f'http://{domain}'  # Assume http for simplicity
        ...
        try:
            response_api = requests.get(url, proxies=proxies)
...
if __name__ == "__main__":
    ...
    serve_transform_classes([ExtractDataDomainTor])
